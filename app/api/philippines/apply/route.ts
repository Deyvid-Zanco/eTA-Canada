...
export async function POST(req: NextRequest) {
  try {
    ...
    await resend.emails.send({
      from: 'Philippines eTravel Application <noreply@immicenter-online.com>',
      to: process.env.ADMIN_EMAIL || 'admin@yourdomain.com',
      subject: `New Philippines eTravel Application - ${data.given_name || ''} ${data.surname || ''}`,
      html: emailHtml,
    });
    ...
    if (data.email) {
      await resend.emails.send({
        ...
        to: data.email,
        subject: 'We have received your response for PHILIPPINES ETRAVEL VISA AUTHORIZATION',
        html: `
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:8px;box-shadow:0 2px 8px rgba(0,0,0,0.04);margin:40px 0;">
        <tr>
          <td style="padding:40px 32px 32px 32px;text-align:center;">
            <h1 style="color:#34A853;font-size:24px;margin-bottom:16px;">We have received your response for PHILIPPINES ETRAVEL VISA AUTHORIZATION</h1>
            <p style="font-size:16px;color:#222;margin-bottom:16px;">Hello!</p>
            <p style="font-size:16px;color:#222;margin-bottom:16px;">We have received your request for the issuance of the Electronic Travel Authorization (eTravel) for the Philippines.</p>
