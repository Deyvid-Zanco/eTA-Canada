...
import { PhilippinesStep1 } from "@/app/components/forms/PhilippinesStep1";
import { PhilippinesStep2 } from "@/app/components/forms/PhilippinesStep2";
...
type Step1FormData = InferType<typeof philippinesStep1Schema>;
type Step2FormData = InferType<typeof philippinesStep2Schema>;
...
function ApplyFormMultiStep() {
  ...
  const step1Methods = useForm<Step1FormData>({
    resolver: yupResolver(philippinesStep1Schema) as Resolver<Step1FormData>,
    mode: 'onSubmit'
  });

  const step2Methods = useForm<Step2FormData>({
    resolver: yupResolver(philippinesStep2Schema) as Resolver<Step2FormData>,
    mode: 'onSubmit'
  });
  ...
  const onSubmit = async (data: Step2FormData) => {
    ...
    try {
      ...
      const response = await fetch('/api/philippines/apply', {
        method: 'POST',
    ...
    <div ref={formRef} className="max-w-4xl mx-auto bg-white shadow-lg rounded-lg p-8 space-y-8">
        ...
        <FormProvider {...step1Methods}>
          <form onSubmit={step1Methods.handleSubmit(nextStep)} noValidate>
            ...
            <div className="flex justify-end mt-8">
              <button
                type="submit"
                className="bg-blue-600 hover:bg-blue-700 text-white py-2 px-12 rounded-md text-lg font-semibold"
              >
                Next
              </button>
            </div>
        ...
        <FormProvider {...step2Methods}>
          <form onSubmit={step2Methods.handleSubmit(onSubmit)} noValidate>
            ...
            <div className="flex justify-between items-center mt-8">
              ...
              <button
                type="submit"
                className="bg-blue-600 hover:bg-blue-700 text-white py-2 px-12 rounded-md text-lg font-semibold disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      {submitStatus === 'success' && (
        <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg text-green-800 font-medium">
          Application submitted successfully! You will be redirected to payment.
        </div>
      )}
      {submitStatus === 'error' && (
    ...
    <>
      ...
      <main className="container mx-auto py-12 px-4">
        ...
        <h1 className="text-3xl font-bold mb-8 text-center">
          Philippines eTravel Application
        </h1>
        
        <div className="text-center mb-8">
          <p className="text-lg text-gray-700">
            Please fill out the form below to apply for your Philippines eTravel authorization.
          </p>
        </div>
        ...
        <div className="max-w-4xl mx-auto mb-8 text-center">
          <h2 className="text-lg font-bold text-gray-800 mb-4">If you apply with us:</h2>
          <div className="space-y-3">
            <div className="flex items-start gap-3 justify-center">
              <div className="w-5 h-5 bg-green-600 rounded flex items-center justify-center flex-shrink-0 mt-0.5">
                <span className="text-white text-xs">✓</span>
              </div>
              <p className="text-gray-700">We review your application before submission</p>
            </div>
            <div className="flex items-start gap-3 justify-center">
              <div className="w-5 h-5 bg-green-600 rounded flex items-center justify-center flex-shrink-0 mt-0.5">
                <span className="text-white text-xs">✓</span>
              </div>
              <p className="text-gray-700">We provide email support throughout the process</p>
            </div>
          </div>
        </div>
        
        <div className="max-w-4xl mx-auto mb-8 text-center">
          <h2 className="text-lg font-bold text-gray-800 mb-4">Permit Renewal</h2>
          <div className="flex items-start gap-3 justify-center">
            <div className="w-5 h-5 bg-green-600 rounded flex items-center justify-center flex-shrink-0 mt-0.5">
              <span className="text-white text-xs">✓</span>
            </div>
            <p className="text-gray-700">Your eTravel authorization is valid for multiple entries within 30 days of approval</p>
          </div>
        </div>
